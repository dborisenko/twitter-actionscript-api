	...
	public class OAuthRequest
	{
				...
				case RESULT_TYPE_HEADER:
					...
					for (var param:Object in _requestParams) {
						...
						if (param.toString().indexOf("oauth") == 0) {
							data += "," + param + "=\"" + com.dborisenko.utils.URLUtil.encode(_requestParams[param]) + "\"";
						}
					}
					
					if (data.charAt(0) == ",")
						data = "OAuth " + data.substr(1);
					else
						data = "OAuth " + data;
		...
		private function getSignableParameters():String {
			...
			for (var param:String in _requestParams) {
				if (param != "oauth_signature")
					aParams.push(param + "=" + URLUtil.encode(_requestParams[param].toString()));
			}
		...
		*/
		public function getSignableString():String {
			...
			var ret:String = URLUtil.encode(_httpMethod.toUpperCase());
			ret += "&";
			ret += URLUtil.encode(_requestURL);
			ret += "&";
			ret += URLUtil.encode(getSignableParameters());
